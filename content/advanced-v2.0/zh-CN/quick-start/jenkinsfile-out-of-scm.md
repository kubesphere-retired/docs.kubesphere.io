...
|---|---|---|---|---|
|Dev| IP : 30861 (`${IP}:${NODEPORT}`)| kubesphere-sample-dev| ks-sample-dev|ks-sample-dev|
...
```shell
# curl {$Virtual IP}:{$Port} 或者 curl {$内网 IP}:{$Port}
...
至此，图形化构建流水线的示例已经完成了，若创建过程中遇到问题，可参考 [常见问题](../../faq)。
