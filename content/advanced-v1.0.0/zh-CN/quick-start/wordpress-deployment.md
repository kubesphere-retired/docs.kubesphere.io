...
1.1. 在当前项目下左侧菜单栏的 **配置中心** 选择 **配置**，点击创建。
...
1.3. ConfigMap 是以键值对的形式存在，此处键值对设置为 `WORDPRESS_DB_PASSWORD` 和 `123456`，点击创建。
...
5.1. 容器组模板中，名称可自定义，镜像填写 `wordpress:4.8-apache`。
...
6.2. 设置存储卷的挂载路径，其中挂载选项选择 **读写**，挂载路径为 `/var/www/html`，点击 **下一步**。

![存储卷设置](/wordpress-pvc-path.png)

#### 第七步：查看部署

7.1. 为方便识别此应用，我们标签设置如下。下一步的节点选择器可以指定容器组调度到期望运行的节点上，此处暂不作设置，点击创建，部署创建完成。
...
8.1. 在当前项目中，左侧菜单选择 **网路与服务 → 服务**，点击 **创建**。
...
8.4. 添加标签并保存，本示例标签设置如下，添加后选择下一步。

```
app=wordpress-service
```

8.5. 服务暴露给外网访问的访问方式，支持 NodePort 和 LoadBalancer，由于示例将以应用路由 (Ingress) 的方式添加 Hostname 来暴露服务给外网访问，所以服务的访问方式选择 **None**。点击创建，wordpress-service 服务可创建成功。
...
- Hostname：可自定义，比如设置为 `kubesphere.wordpress.io`，它是应用路由规则的访问域名，最终使用此域名来访问对应的服务 (如果访问入口是以 NodePort 的方式启用，需要保证 Hostname 能够在客户端正确解析到集群工作节点上；如果是以 LoadBalancer 方式启用，需要保证 Hostname 正确解析到负载均衡器的 IP 上)。
...
9.4. 本示例注解 (Annotation) 设置如下所示，Annotation 是用户任意定义的“附加”信息，以便于外部工具进行查找，详见 [Annotation 官方文档](https://kubernetes.io/docs/concepts/overview/working-with-objects/annotations/)。完成后选择下一步：
...
10.2. 网关入口提供提供如下两种方式，本示例以 `NodePort` 访问方式为例配置网关入口，点击保存。
...
10.3. 外网访问设置后，将产生两个节点端口 (NodePort)，分别是 HTTP 协议的端口和 HTTPS 协议的节点端口，比如这里依次是 `30517` 和 `30409`。

> 注意：创建应用路由之后应该把主机的公网 IP 和 配置的域名如：`139.198.17.33 kubesphere.wordpress.io` 填入本地的 `hosts` 配置文件中，才可以通过浏览器访问。注意，可能需要进行端口转发和防火墙放行对应的端口，保证外网流量能够通过需要访问的主机端口，否则外网无法访问。

![端口列表](/gateway-nodeport-list.png)

### 访问 Wordpress
至此，WordPress 就以应用路由的方式通过网关入口暴露到外网，由于在路由规则中我们选择的是 http 协议，因此可以通过示例中在路由规则和外网访问配置的 `{$hostname}{$NodePort}` 如 `kubesphere.wordpress.io:30517` 访问 WordPress 博客网站。
